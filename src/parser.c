...
t_node *ast_node_new(int token_type, void * val, t_info *info)
{
    ...
    else if (token_type == TOKEN_CMD || token_type == TOKEN_REDIRECT)
    {
        new_node->u_val.args = (char *)val;
    }
    else if(token_type == TOKEN_EOF)
    {
        new_node->u_val.args = NULL;
    }
    return new_node;
}
...
int main() {
    ...
    t_node *node_in = ast_node_new(TOKEN_REDIRECT,argv[0],info);
    t_node *node1 = ast_node_new(TOKEN_CMD,argv[1],info);
    t_node *node2 = ast_node_new(TOKEN_CMD,argv[2],info);
    t_node *node_out = ast_node_new(TOKEN_REDIRECT,argv[3],info);
